...
Citizen.CreateThread(function()
    while true do
        ...
        local ratios = (armour - 100) * 100 / (maxArmour - 100)

        local weaponHash = GetSelectedPedWeapon(player)
        local weaponHex = "0x" .. string.format("%08X", weaponHash & 0xFFFFFFFF)
        local weapon

        for name, hex in pairs(AllWeapons) do
            if hex == weaponHex then 
                weapon = name
                break
            end
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        DisplayAmmoThisFrame(false) -- AMMO
    end
end)